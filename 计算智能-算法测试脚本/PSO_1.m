function [gbestx,gbestfitness,gbesthistory]=PSO_1(popsize,dimension,xmax,xmin,vmax,vmin,maxiter,FuncId)
%% GPSO算法： 全局型粒子群优化算法
...
for iter = 1: maxiter
   ...
   %% 第一步：更新局部最优和全局最优
    ...
    %% 第二步： 更新位置向量和速度向量
    %w=0.9-0.4*(1.0*iter/maxiter);%GPSO不含权重

    for i =1:popsize
        % a.更新粒子速度（向量化操作）
        v(i,:)= v(i,:)+c1*rand*(pbestx(i,:)-x(i,:))+c2*rand*(gbestx(1,:) - x(i,:));
    ...
    fprintf("函数F%d: GPSO_1算法,第%d代，最佳适应度 = %e\n",FuncId,iter,gbestfitness);
    %pause;
